  ...
  <div class="questions" v-infinite-scroll='loadMore'> 
     <ul class="question-list"> 
      <li v-for="(item,index) in items" :key="index"> 
        <span class="fl good"><span class="num">{{item.replys}}</span> 回答</span> 
        <span class="title"><router-link :to="'/qa/detail/'+item.id">{{item.title}}</router-link></span> 
...
export default {
  ...
  methods:{
    fetchData(){
      this.pageNo=1;
      questionApi.searchByUserid(this.pageNo,12,this.$route.params.id).then(res=>{
